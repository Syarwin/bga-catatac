...
define(['dojo', 'dojo/_base/declare', g_gamethemeurl + 'modules/js/vendor/nouislider.min.js', 'ebg/core/gamegui'], (
  ...
  return declare('customgame.game', ebg.core.gamegui, {
    ...
    takeAction(action, data, check = true, checkLock = true) {
      ...
      data = data || {};
      let lock = data.lock === undefined ? false : data.lock;
      delete data.lock;
      return this.bgaPerformAction(action, data, { checkAction: check, lock });
    },
    ...
    /**
     * getTiming: useful for notif + handle gamepref
     */
    getTiming(t) {
      if (this.isFastMode()) {
        return 0;
      ...
      // Interpolate based on setting, 3 is normal speed
      let speed = this.prefs ? this.prefs[111].value : 3;
      return (speed * t) / 3;
    },

    wait(n, speedAjustable = true) {
      return new Promise((resolve, reject) => {
        if (this.isFastMode()) {
          resolve();
        } else {
          setTimeout(() => resolve(), speedAjustable ? this.getTiming(n) : n);
        }
      });
    },
    ...
    slide(mobileElt, targetElt, options = {}) {
      ...
      if (config.phantomEnd) {
        ...
        $(targetId).style.removeProperty('position');
        $(targetId).style.removeProperty('left');
        $(targetId).style.removeProperty('top');
